...
while True:
    ...
    """计算奖励"""
    makespan = 0.0
    t_start, n_active = submission_time, env.active_workflow_count

    env_log = env.step(server_name, server_id, numa_node_id, memory)

    for t_end in sorted(r.completion_time for r in env_log.finished_workflow_records):
        assert t_end >= t_start
        makespan += n_active * (t_end - t_start)
        t_start = t_end
        n_active -= 1

    if (n_active > 0) and ((t_end := env.current_function.submission_time) >= t_start):
        makespan += n_active * (t_end - t_start)

    reward = -WEIGHT * makespan - (1 - WEIGHT) * SCALE * env_log.rent
    print(f"奖励 = {reward:.6f} (时间项 = {makespan:.6f}, 成本项 = {SCALE * env_log.rent:.6f})")
